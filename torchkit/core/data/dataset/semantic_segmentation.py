...
from torchkit.core.vision import draw_semantic_mask
from torchkit.core.vision import get_image_size
from torchkit.core.vision import is_image_file
...
class SemanticSegmentationDataset(VisionDataset, metaclass=ABCMeta):
    ...
    def get_item(self, index: int) -> tuple[Tensor, Tensor, Dim3]:
        ...
        target = to_tensor(target, normalize=False).to(torch.uint8)
    ...
    def load_mosaic(self, index: int) -> tuple[np.ndarray, np.ndarray]:
        ...
        # NOTE: Augment
        """
        input4, target4 = paired_random_perspective(
            image1= input4,
            image2= target4,
    ...
    def collate_fn(batch) -> tuple[Tensor, Tensor, Dim3]:
        ...
        return torch.stack(input, 0), torch.stack(target, 0), shapes
