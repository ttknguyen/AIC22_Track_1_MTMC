...
from torchkit.core.vision import bbox_xywh_to_cxcywh_norm
...
class CocoDetectionLabelHandler(BaseLabelHandler):
	...
	def load_from_file(
		self, image_paths: list[str], label_path: str, **kwargs
	) -> dict[str, VisionData]:
		...
		if not is_json_file(label_path):
			raise ValueError(f"{label_path} is not a JSON file.")
		...
		with progress_bar() as pbar:
			for a in pbar.track(
				...
				bbox_cxcywh_norm = bbox_xywh_to_cxcywh_norm(box_xywh, height0, width0)
