...
from torchkit.core.vision import batch_bbox_iou
...
class SORT(BaseTracker):
	...
	def update(self, instances: list[Instance]):
		...
		if len(instances) > 0:
			...
			insts = np.array(
				[np.append(np.float64(d.bbox), np.float64(d.confidence))
				 for d in instances]
			)
		else:
			insts = np.empty((0, 5))
		...
		self.update_matched_tracks(
			matched_indexes=matched_indexes, instances=instances
		)

		# NOTE: Create and initialise new trackers for unmatched detections
		self.create_new_tracks(
			unmatched_inst_indexes=unmatched_inst_indexes, instances=instances
		)
	...
	def associate_instances_to_tracks(
		...
		iou_matrix = batch_bbox_iou(instances, tracks)
	...
	def update_matched_tracks(
		self,
		matched_indexes: Union[list, np.ndarray],
		instances	   : list[Instance]
	):
	...
	def create_new_tracks(
		self,
		unmatched_inst_indexes: Union[list, np.ndarray],
		instances			  : list[Instance]
	):
		"""Create new tracks.
		...
		for i in unmatched_inst_indexes:
			...
			new_trk = MovingObject(
				instance = instances[i],
				motion   = self.motion_model(bbox=instances[i].bbox)
			)
