...
from torchkit.core.factory import PADDING_LAYERS
...
def autopad(kernel_size: Size2T, padding: Union[str, Size2T, None] = None):
    """Pad to `same`."""
    if padding is None:
...
def get_padding_value(
    padding: Union[str, Size2T, None], kernel_size: Size2T, **kwargs
...
def get_same_padding(
    input: int, kernel_size: int, stride: int, dilation: int
) -> int:
    """Calculate asymmetric TensorFlow-like 'same' padding for a convolution.
    """
    return max((math.ceil(input / stride) - 1) * stride +
               (kernel_size - 1) * dilation + 1 - input, 0)
