...
from torchkit.core.file import is_torch_saved_file
from torchkit.core.utils import console
from torchkit.core.utils import error_console
from torchkit.utils import root_dir
...
class CheckpointCallback(Callback):
	...
	def init_checkpoint_dir(
		self, model_dir: str, version: Optional[Union[int, str]]
	):
	...
	def validate_init_configuration(self):
		...
		if self.every_n_epochs < 0:
			raise ValueError(
				f"Invalid value for `every_n_epochs={self.every_n_epochs}`."
				f" Must be >= 0."
			)
	...
	def on_load_checkpoint(
		...
		if self.best_checkpoint_path == "" and self.best_checkpoint_relpath != "":
			self.best_checkpoint_path = os.path.join(root_dir, self.best_checkpoint_relpath)
		...
		if self.last_checkpoint_path == "" and self.last_checkpoint_relpath != "":
			self.last_checkpoint_path = os.path.join(root_dir, self.last_checkpoint_relpath)
	...
	def _save_best_checkpoint(
		...
		sorted_dict = dict(
			sorted(ckpt_dict.items(), key=lambda x: x[1], reverse=reverse)
		)
		...
		if best_path != self.best_checkpoint_path:
			...
			self.best_checkpoint_relpath = os.path.relpath(
				self.best_checkpoint_path, root_dir
			)
			...
			if self.verbose:
				...
				if trainer.is_global_zero:
					...
					console.log(
						...
						f"Save checkpoint: {self.best_checkpoint_relpath}"
					)
	...
	def _save_last_checkpoint(
		...
		if (
			...
			self._del_model(
				trainer=trainer, filepath=self.last_checkpoint_path
			)
		
		self.last_checkpoint_path    = filepath
		self.last_checkpoint_relpath = os.path.relpath(
			self.last_checkpoint_path, root_dir
		)
			...
			if current != self.best_score:
				...
				if trainer.is_global_zero:
					...
					console.log(
						...
						f"Save checkpoint: {self.last_checkpoint_relpath}"
					)
