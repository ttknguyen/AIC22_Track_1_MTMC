...
from torchkit.core.factory import HEADS
...
class ClassifierHead(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        if self.drop_rate:
            input = F.dropout(
                input, p=float(self.drop_rate), training=self.training
            )
        input = self.fc(input)
...
class Detect(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        for i in range(self.num_layers):
            input[i] = self.m[i](input[i])      # Conv
            bs, _, ny, nx = input[i].shape  # input(bs,255,20,20) to input(bs,3,20,20,85)
            input[i] = input[i].view(bs, self.num_anchors, self.num_outputs, ny, nx)
            input[i] = input[i].permute(0, 1, 3, 4, 2).contiguous()

            if not self.training:  # inference
                if self.grid[i].shape[2:4] != input[i].shape[2:4]:
                    self.grid[i] = self._make_grid(nx, ny).to(input[i].device)
                
                y = input[i].sigmoid()
                y[..., 0:2] = (y[..., 0:2] * 2.0 - 0.5 +
                               self.grid[i].to(input[i].device)) * self.stride[i]  # xy
                y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh
                z.append(y.view(bs, -1, self.num_outputs))
        
        return input if self.training else (torch.cat(z, 1), input)
