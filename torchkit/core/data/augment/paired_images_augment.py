...
from torchkit.core.vision import image_channels
from torchkit.core.vision import to_tensor
from torchkit.core.vision.utils.image import get_image_size
from .base import BaseAugment
from .base import BaseAugmentModule
from .utils import apply_transform_op
...
# MARK: - PairedImagesAugmentModule
...
class PairedImagesAutoAugment(BaseAugment):
    ...
    cfgs = {
        "imagenet": [
            ...
            (("shear_x"  , 0.6, 5   , 10)  , ("equalize"     , 1.0, None, None)),
        ...
        "cifar10": [
            ...
            (("rotate"       , 0.7, 2   , 10)  , ("translate_x"  , 0.3, 9   , 10)),
            (("sharpness"    , 0.8, 1   , 10)  , ("sharpness"    , 0.9, 3   , 10)),
            (("shear_y"      , 0.5, 8   , 10)  , ("translate_y"  , 0.7, 9   , 10)),
            (("auto_contrast", 0.5, None, None), ("equalize"     , 0.9, None, None)),
            (("shear_y"      , 0.2, 7   , 10)  , ("posterize"    , 0.3, 7   , 10)),
            ...
            (("color"        , 0.7, 7   , 10)  , ("translate_x"  , 0.5, 8   , 10)),
            (("equalize"     , 0.3, None, None), ("auto_contrast", 0.4, None, None)),
            (("translate_y"  , 0.4, 3   , 10)  , ("sharpness"    , 0.2, 6   , 10)),
            ...
            (("translate_y"  , 0.9, 9   , 10)  , ("translate_y"  , 0.7, 9   , 10)),
            (("auto_contrast", 0.9, None, None), ("solarize"     , 0.8, 3   , 10)),
            (("equalize"     , 0.8, None, None), ("invert"       , 0.1, None, None)),
            (("translate_y"  , 0.7, 9   , 10)  , ("auto_contrast", 0.9, None, None)),
        ],
        "svhn": [
            # (op_name, p, magnitude, num_magnitude_bins=10)
            (("shear_x" , 0.9, 4   , 10)  , ("invert"       , 0.2, None, None)),
            (("shear_y" , 0.9, 8   , 10)  , ("invert"       , 0.7, None, None)),
            ...
            (("shear_x" , 0.9, 4   , 10)  , ("auto_contrast", 0.8, None, None)),
            (("shear_y" , 0.9, 8   , 10)  , ("invert"       , 0.4, None, None)),
            (("shear_y" , 0.9, 5   , 10)  , ("solarize"     , 0.2, 6   , 10)),
            (("invert"  , 0.9, None, None), ("auto_contrast", 0.8, None, None)),
            (("equalize", 0.6, None, None), ("rotate"       , 0.9, 3   , 10)),
            (("shear_x" , 0.9, 4   , 10)  , ("solarize"     , 0.3, 3   , 10)),
            (("shear_y" , 0.8, 8   , 10)  , ("invert"       , 0.7, None, None)),
            (("equalize", 0.9, None, None), ("translate_y"  , 0.6, 6   , 10)),
            (("invert"  , 0.9, None, None), ("equalize"     , 0.6, None, None)),
            (("contrast", 0.3, 3   , 10)  , ("rotate"       , 0.8, 4   , 10)),
            (("invert"  , 0.8, None, None), ("translate_y"  , 0.0, 2   , 10)),
            (("shear_y" , 0.7, 6   , 10)  , ("solarize"     , 0.4, 8   , 10)),
            (("invert"  , 0.6, None, None), ("rotate"       , 0.8, 4   , 10)),
            (("shear_y" , 0.3, 7   , 10)  , ("translate_x"  , 0.9, 3   , 10)),
            (("shear_x" , 0.1, 6   , 10)  , ("invert"       , 0.6, None, None)),
            (("solarize", 0.7, 2   , 10)  , ("translate_y"  , 0.6, 7   , 10)),
            (("shear_y" , 0.8, 4   , 10)  , ("invert"       , 0.8, None, None)),
            (("shear_x" , 0.7, 9   , 10)  , ("translate_y"  , 0.8, 3   , 10)),
            (("shear_y" , 0.8, 5   , 10)  , ("auto_contrast", 0.7, None, None)),
            (("shear_x" , 0.7, 2   , 10)  , ("invert"       , 0.1, None, None)),
        ],
        "enhancement": [
            # (op_name, p, magnitude, num_magnitude_bins=10)
            (("fliplr", 0.5, None, None),),
            (("flipud", 0.5, None, None),),
            (("rotate", 0.5, 0, 4),),
            (("rotate", 0.5, 1, 4),),
            (("rotate", 0.5, 2, 4),),
            (("rotate", 0.5, 3, 4),),
            (("rotate", 0.5, 0, 4), ("fliplr", 0.5, None, None)),
            (("rotate", 0.5, 1, 4), ("fliplr", 0.5, None, None)),
            (("rotate", 0.5, 2, 4), ("fliplr", 0.5, None, None)),
            (("rotate", 0.5, 3, 4), ("fliplr", 0.5, None, None)),
            (("rotate", 0.5, 0, 4), ("flipud", 0.5, None, None)),
            (("rotate", 0.5, 1, 4), ("flipud", 0.5, None, None)),
            (("rotate", 0.5, 2, 4), ("flipud", 0.5, None, None)),
            (("rotate", 0.5, 3, 4), ("flipud", 0.5, None, None)),
        ]
    }
    ...
    def _augmentation_space(
        ...
        return {
            ...
            "shear_x"      : (torch.linspace(0.0, 0.3,   num_bins), True),
            "shear_y"      : (torch.linspace(0.0, 0.3,   num_bins), True),
            "solarize"     : (torch.linspace(255.0, 0.0, num_bins), False),
            "translate_x"  : (torch.linspace(0.0, 150.0 / 331.0 * image_size[0], num_bins), True),
            "translate_y"  : (torch.linspace(0.0, 150.0 / 331.0 * image_size[1], num_bins), True),
        }
    ...
    def forward(
        ...
        Args:
            ...
            target (PIL Image or Tensor, optional):
        ...
        if isinstance(input, Tensor):
            if isinstance(fill, (int, float)):
                fill = [float(fill)] * image_channels(input)
        ...
        for i, (op_name, p, magnitude, num_magnitude_bins) in enumerate(
            ...
            if probs[i] <= p:
                ...
                input = apply_transform_op(
                    input, op_name, magnitude, interpolation=self.interpolation,
                    fill=fill
                )
                if target is not None:
                    target = apply_transform_op(
                        target, op_name, magnitude,
...
class PairedImagesRandAugment(BaseAugment):
    ...
    def forward(
        ...
        for _ in range(self.num_ops):
            op_meta   = self._augmentation_space(
                self.num_magnitude_bins, get_image_size(input)
            )
            ...
            if target is not None:
                target = apply_transform_op(
                    target, op_name, magnitude, interpolation=self.interpolation,
                    fill=fill
                )
...
# MARK: - PairedImagesTrivialAugmentWide
...
class PairedImagesTrivialAugmentWide(BaseAugment):
    ...
    def _augmentation_space(
        ...
        return {
            ...
            "shear_x"      : (torch.linspace(0.0, 0.99,  num_bins), True),
            "shear_y"      : (torch.linspace(0.0, 0.99,  num_bins), True),
            "solarize"     : (torch.linspace(255.0, 0.0, num_bins), False),
            "translate_x"  : (torch.linspace(0.0, 32.0,  num_bins), True),
            "translate_y"  : (torch.linspace(0.0, 32.0,  num_bins), True),
        }
