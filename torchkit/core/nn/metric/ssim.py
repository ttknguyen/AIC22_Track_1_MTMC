...
from torchkit.core.vision import filter2d
from torchkit.core.vision import get_gaussian_kernel2d
...
def ssim(
    ...
    if not isinstance(image1, Tensor):
        raise TypeError(f"Input img1 type is not a Tensor. "
                        f"Got: {type(image1)}")
    if not isinstance(image2, Tensor):
        raise TypeError(f"Input img2 type is not a Tensor. "
                        f"Got: {type(image2)}")
    if not isinstance(max_val, float):
        raise TypeError(f"Input max_val type is not a float. "
                        f"Got: {type(max_val)}")
    if not len(image1.shape) == 4:
        raise ValueError(f"Invalid img1 shape, we expect [B, C, H, W]. "
                         f"Got: {image1.shape}")
    if not len(image2.shape) == 4:
        raise ValueError(f"Invalid img2 shape, we expect [B, C, H, W]. "
                         f"Got: {image2.shape}")
    if not image1.shape == image2.shape:
        raise ValueError(f"img1 and img2 shapes must be the same. "
