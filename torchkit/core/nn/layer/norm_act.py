...
from torchkit.core.factory import NORM_ACT_LAYERS
from torchkit.core.type import Callable
from .act import create_act_layer
from .evo_norm import EvoNormBatch2d
from .evo_norm import EvoNormSample2d
...
class BatchNormAct2d(nn.BatchNorm2d):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        if torch.jit.is_scripting():
            input = self._forward_jit(input)
        else:
            input = self._forward_python(input)
        input = self.act(input)
        return input

    def _forward_jit(self, input: Tensor) -> Tensor:
    ...
    @torch.jit.ignore
    def _forward_python(self, input: Tensor) -> Tensor:
        return super(BatchNormAct2d, self).forward(input)
